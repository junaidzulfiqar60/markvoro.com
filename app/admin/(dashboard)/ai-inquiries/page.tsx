...
export default function AiInquiriesPage() {
  ...
  const load = () => {
    ...
    fetch(`/api/admin/ai-inquiries?${params}`)
      .then((res) => res.json())
      .then((data) => {
        setItems(data.items || []);
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/admin/ai-inquiries/${selected.id}`, {
        method: "PATCH",
  ...
  const remove = async (id: string) => {
    await fetch(`/api/admin/ai-inquiries/${id}`, { method: "DELETE" });
  ...
  const columns: Column<Inquiry>[] = [
    ...
    { key: "agentType", label: "Agent Type", render: (row) => row.agentType.replace(/_/g, " ") },
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="font-display text-2xl font-bold text-white">AI Agent Inquiries</h1>
        <p className="mt-1 text-sm text-white/50">Businesses interested in AI automation.</p>
      </div>
      ...
      <DataTable
        ...
        emptyMessage="No AI agent inquiries yet."
      />

      <Modal open={!!selected} onClose={() => setSelected(null)} title="AI Agent Inquiry">
        {selected && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                <p className="text-xs text-white/40">AI Solution</p>
                <p className="text-white">{selected.agentType.replace(/_/g, " ")}</p>
              </div>
              <div>
                <p className="text-xs text-white/40">Est. Monthly Customers</p>
                <p className="text-white">{selected.estimatedMonthlyCustomers || "—"}</p>
              </div>
              <div>
                <p className="text-xs text-white/40">Preferred Contact</p>
                <p className="text-white">{selected.preferredCommunication}</p>
              </div>
            </div>
            <div>
              <p className="text-xs text-white/40">Current Process</p>
              <p className="mt-1 text-sm text-white/80">{selected.currentProcess || "—"}</p>
            </div>
            <div>
              <p className="text-xs text-white/40">Business Problem / Tasks to Automate</p>
              <p className="mt-1 text-sm text-white/80">{selected.businessProblem}</p>
            </div>
            {selected.message && (
              <div>
                <p className="text-xs text-white/40">Additional Requirements</p>
