...
interface InstagramWebhookPayload {
  entry?: Array<{
    id: string; // IG-scoped business account ID
...
export class InstagramController {
  ...
  async receive(@Req() req: Request, @Res() res: Response) {
    ...
    for (const entry of payload.entry ?? []) {
      const client = await this.channels.findClientByIgBusinessId(entry.id);
      if (!client) continue;
      ...
      for (const event of entry.messaging ?? []) {
        ...
        if (replyText) {
          const token = this.channels.getInstagramToken(client);
